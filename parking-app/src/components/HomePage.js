...
const HomePage = () => {
    ...
    const  generateCars = async (token) => {
        ...
        try {
            let generatedVehicles = await axios.post('http://199.17.162.16:5000/api/genCars', { token });
            console.log(typeof(generatedVehicles)+"data" +generatedVehicles.data)
            let vehicles = [];
            JSON.parse(generatedVehicles.data).forEach(vehicle => {
                vehicles.push(<Vehicle key={vehicle.permitNum} make={vehicle.make} model={vehicle.model} color={vehicle.color} liscensePlate={vehicle.plate} state={vehicle.state} status={vehicle.status} navigate={navigate}/>)
            });
...
const Vehicle = ({navigate, make, model, color, liscensePlate, state, status}) => {
    ...
    return (
        <div className={(status === 'accepted') ? 'vehicle accepted' : (status === 'denied') ? 'vehicle denied' : 'vehicle pending'}>
            ...
            <div className="status-label" style={{float: "right", marginRight: 20}}>{status.toUpperCase()}</div>
